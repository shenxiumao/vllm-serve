...
# 创建API Key管理将在MODEL_BACKENDS定义后进行

app = FastAPI(
    title="AI Model Proxy API",
    description="Proxy server for multiple AI models with API key authentication",
    version="1.1.0",
    openapi_tags=[
        {
            "name": "Models",
...
# 定义模型名称到后端URL的映射
MODEL_BACKENDS = {
    "DeepSeek-R1-0528-Qwen3-14B": "http://localhost:8000",
    "Qwen3-14B": "http://localhost:8001",
    "DeepSeek-R1-Distill-Qwen-7B": "http://localhost:8002",
    "Qwen2.5-7B-Instruct": "http://localhost:8003"
}
...
async def forward_stream(request: Request, backend_url: str, body: dict, headers: dict):
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        ...
        async with client.stream(
            ...
            # 记录流式传输开始
            logger.info(f"Starting stream forwarding to client")
            ...
            # 记录流式传输完成
            logger.info(f"Stream forwarding completed")

@app.api_route("/v1/{endpoint:path}", methods=["GET", "POST", "OPTIONS", "HEAD"], tags=["Models", "Chat"])
...
def run_server():
    ...
    config = uvicorn.Config(
        app="proxy_server:app",  # 使用模块:应用格式
        host="0.0.0.0",
        port=1024,
